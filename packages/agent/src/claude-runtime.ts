...
interface ClaudeInternalRunOptions extends ClaudeRunOptions {
  ...
  modelFactsFn?: (provider: string, model: string) => CatalogModelFacts | undefined;
}
...
async function runClaudeAgentWithOptions(
  ...
  const selectedFacts = (options.modelFactsFn ?? catalogModelFacts)("anthropic", selected);
  if (
    selectedFacts === undefined ||
    (selectedFacts.lifecycle !== "reviewed" && selectedFacts.lifecycle !== "routable") ||
    selectedFacts.messagesAPI !== "supported"
  ) {
    throw new Error(`cave_claude_model_not_runtime_eligible:${selected}`);
  }
...
function claudeReasoningOptions(
  ...
  const capability = claudeThinkingCapability(facts);
...
function claudeThinkingCapability(
  facts: CatalogModelFacts,
): "adaptive" | "manual" | "unknown" {
  if (facts.adaptiveThinking === "supported") return "adaptive";
  if (facts.manualThinking === "supported") return "manual";
  return "unknown";
}
...
function rejectInternalOptions(options: ClaudeRunOptions): void {
  ...
  if (
    "lockedBuild" in value ||
    "candidatePlan" in value ||
    "queryFn" in value ||
